    ...
    try:
        ...
        if not lro_res.get("done") and operation_name:
            ...
            for i in range(max_polls):
                ...
                from utils.proxy import get_chrome_client
...
async def _sync_antigravity(credential: OAuthCredential, session: AsyncSession) -> dict:
    """
    同步 Antigravity 客户端数据。
    
    端点选择逻辑（逆向自官方 Antigravity main.js L382949-394395）：
      官方 Antigravity 的 w() 方法每次请求时都从 token 读取 isGcpTos，
      然后由 IYs() 函数选择端点：
        - isGcpTos=true  → cloudcode-pa.googleapis.com (production)
        - isGcpTos=false → daily-cloudcode-pa.googleapis.com
      
      isGcpTos 的值在 OAuth 登录时就已经确定，取决于使用的 OAuth Client ID：
        - Non-GCP: L6t = "1071006060591-..." → isGcpTos=false → daily endpoint
        - GCP ToS: M6t = "884354919052-..." → isGcpTos=true  → production endpoint
      
      NullGravity 使用的是非 GCP OAuth Client（1071006060591-...），
      所以所有请求都应该走 daily 端点。
      
      此前的 bug 是从 loadCodeAssist 响应中读取 usesGcpTos 来决定 endpoint，
      导致 token=non-GCP-ToS 却去访问 production 端点 → 403 PERMISSION_DENIED。
    
    loadCodeAssist 后续调用逻辑（L394362-394372）：
      1. 先正常调用
      2. 如果 !paidTier && cloudaicompanionProject != "" → 再调一次
      3. 从响应中提取 cloudaicompanionProject 作为后续 API 的 project 参数
    """
    access_token = credential.access_token
    if not access_token:
        return {"success": False, "error": "No access token"}

    # 端点选择基于 credential 的 is_gcp_tos 属性（登录时由 OAuth Client ID 决定）
    # 而不是从 loadCodeAssist 响应中动态判断。
    # 详见官方 w() 方法: this.u(f) 中 f 来自 token 存储，不是 API 响应。
    is_gcp_tos = getattr(credential, 'is_gcp_tos', False) or False
    logger.info(f"[Sync-Antigravity] Using is_gcp_tos={is_gcp_tos} from credential (OAuth login context)")

    try:
        # Step 1: loadCodeAssist
        # 官方逻辑: 先用 token 对应的端点调用，如果没有 paidTier 但有 project，再调一次
        ...
        logger.info(f"[Sync-Antigravity] loadCodeAssist result keys: {list(load_res.keys())}")

        tier_obj = load_res.get("paidTier") or load_res.get("currentTier") or {}
        tier_id = tier_obj.get("id")
        project_id = load_res.get("cloudaicompanionProject")

        # 官方 loadCodeAssist 的双重调用逻辑 (L394367):
        # if (!paidTier && cloudaicompanionProject !== "") → 用 enterprise project 再调一次
        paid_tier = load_res.get("paidTier")
        if not paid_tier and project_id:
            logger.info(f"[Sync-Antigravity] No paidTier but has project={project_id}, re-calling loadCodeAssist")
            try:
                load_res2 = await sandbox_post(
                    access_token, "loadCodeAssist",
                    {"metadata": {"ideType": "ANTIGRAVITY"}},
                    account_id=credential.account_id,
                    is_gcp_tos=is_gcp_tos,
                )
                tier_obj = load_res2.get("paidTier") or load_res2.get("currentTier") or {}
                tier_id = tier_obj.get("id")
                project_id = load_res2.get("cloudaicompanionProject") or project_id
                load_res = load_res2
            except Exception as e:
                logger.warning(f"[Sync-Antigravity] Second loadCodeAssist failed (non-critical): {e}")

        logger.info(f"[Sync-Antigravity] project={project_id}, tier={tier_id}, is_gcp_tos={is_gcp_tos}")

        credential.tier = tier_id
        credential.project_id = project_id

        # Step 2: fetchAvailableModels — 与 loadCodeAssist 使用同一端点
        # 官方 getAvailableModels() 使用 this.w()，它通过 this.u(f) 选端点
        # f 是 token (包含 isGcpTos)，所以端点与 loadCodeAssist 一致
