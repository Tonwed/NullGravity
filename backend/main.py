...
from routers import accounts, settings, auth, logs, dashboard, api_proxy, api_tokens, model_mappings
...
app.include_router(api_proxy.router, prefix="/api/api-proxy", tags=["api-proxy"])
app.include_router(api_tokens.router, prefix="/api/api-tokens", tags=["api-tokens"])
app.include_router(model_mappings.router, prefix="/api/model-mappings", tags=["model-mappings"])

# Top-level OAuth callback — official Antigravity redirects to /oauth-callback on localhost
from fastapi.responses import HTMLResponse
app.get("/oauth-callback", response_class=HTMLResponse)(_oauth_callback_handler)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:app", host="127.0.0.1", port=8046, reload=False)
